package com.example.odev1firatatar.Odev1

    fun main() {
        
        val sehir: String = "Adana"
        println("Hangi Şehir: $sehir")

        val ulke: String = "Germany"
        println("Hangi Ülke: $ulke")

        val telefon: String = "055421312313"
        println("Telefon Numarası: $telefon")

        val postaKodu: String = "01100"
        println("Posta Kodu: $postaKodu")

        val email: String = "example@mail.com"
        println("Email: $email")

        val meslek: String = "Yazılım Mühendisi"
        println("Meslek: $meslek")

        val stokMiktari: Int = 150
        println("Stok Miktarı: $stokMiktari")

        val musteriAdi: String = "Ali Veli"
        println("Müşteri Adı: $musteriAdi")

        val bakiye: Double = 2500.75
        println("Bakiye: $bakiye")

        val dogumGunu: String = "1995-06-15"
        println("Doğum Günü: $dogumGunu")

        val maas: Double = 12000.0
        println("Maaş: $maas")

        val medeniDurum: String = "Bekar"
        println("Medeni Durum: $medeniDurum")

        val urunYorum: String = "Çok kaliteli bir ürün."
        println("Ürün Yorumu: $urunYorum")

        val odemeTarihi: String = "2025-04-29"
        println("Ödeme Tarihi: $odemeTarihi")

        val odeme: String = "Kredi Kartı"
        println("Ödeme Yöntemi: $odeme")

        val siparisAdeti: Int = 3
        println("Sipariş Adeti: $siparisAdeti")

        val arabaModeli: String = "Toyota Corolla"
        println("Araba Modeli: $arabaModeli")

        val kitapAdi: String = "Simyacı"
        println("Kitap Adı: $kitapAdi")

        val yayinlamaTarihi: String = "2023-01-01"
        println("Yayınlama Tarihi: $yayinlamaTarihi")

        val indirimMiktari: Double = 15.5
        println("İndirim Miktarı: $indirimMiktari")

        val odaSayisi: Int = 5
        println("Oda Sayısı: $odaSayisi")

        val enlem: Double = 37.0
        println("Enlem: $enlem")

        val boylam: Double = 35.321
        println("Boylam: $boylam")

        val urunAdi: String = "Bluetooth Kulaklık"
        println("Ürün Adı: $urunAdi")

        val yemekFiyati: Double = 120.0
        println("Yemek Fiyatı: $yemekFiyati")

        val marka: String = "Samsung"
        println("Marka: $marka")

        val muzikAdi: String = "Imagine"
        println("Müzik Adı: $muzikAdi")

        val videoSuresi: Int = 240
        println("Video Süresi (sn): $videoSuresi")

        val urunPuani: Double = 4.7
        println("Ürün Puanı: $urunPuani")

        val resimAdi: String = "manzara.jpg"
        println("Resim Adı: $resimAdi")

        val dosyaFormati: String = "JPEG"
        println("Dosya Formatı: $dosyaFormati")

        val renk: String = "Mavi"
        println("Renk: $renk")

        val renkKodu: String = "#0000FF"
        println("Renk Kodu: $renkKodu")

        val telefonModeli: String = "iPhone 13"
        println("Telefon Modeli: $telefonModeli")

        val ekranBoyutu: Double = 6.1
        println("Ekran Boyutu (inç): $ekranBoyutu")

        val agirlik: Double = 174.0
        println("Ağırlık (gram): $agirlik")

        val ulusalGun: String = "29 Ekim"
        println("Ulusal Gün: $ulusalGun")

        val tatilGunu: String = "1 Ocak"
        println("Tatil Günü: $tatilGunu")

        val rezervasyonTarihi: String = "2025-05-15"
        println("Rezervasyon Tarihi: $rezervasyonTarihi")

        val sokakAdi: String = "Atatürk Caddesi"
        println("Sokak Adı: $sokakAdi")

        val otobusHatti: String = "42M"
        println("Otobüs Hattı: $otobusHatti")

        val kalanDakika: Int = 8
        println("Kalan Dakika: $kalanDakika")

        val takipKodu: String = "TR123456789"
        println("Takip Kodu: $takipKodu")

        val kuponSuresi: Int = 30
        println("Kupon Süresi (gün): $kuponSuresi")

        val kuponKodu: String = "INDIRIM30"
        println("Kupon Kodu: $kuponKodu")

        val faturaAdresi: String = "İnönü Mah. 123. Sok. No:5"
        println("Fatura Adresi: $faturaAdresi")





}
